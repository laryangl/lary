...
async def ytdl_down(event, opts, url):
    try:
        await event.edit("❆︙- برو صبر شوي جاي ابحوش")
        ...
        await event.edit("❆︙- برو شهل مقطع صغير شوف واحد حجمه زين")
        return None
    except GeoRestrictedError:
        await event.edit(
            "❆︙- واللهي يا برو اكو قيود علي فا اعذرني ماكدر اجيب الي تريده "
        )
        return None
    except MaxDownloadsReached:
        await event.edit("❆︙-والله ضلعي هواي نزلتلك ")
        return None
    except PostProcessingError:
        await event.edit("❆︙ برو اكو خطاء ماكدر اعالجه")
        return None
    except UnavailableVideoError:
        await event.edit("`برو ماكو هيح وسائط بهذا التنسيق`")
        return None
    except XAttrMetadataError as XAME:
        await event.edit(f"`{XAME.code}: {XAME.msg}\n{XAME.reason}`")
        return None
    except ExtractorError:
        await event.edit("❆︙برو اكو خطابل معلومات حطها عدل")
        return None
...
async def download_video(event):
    ...
    if not url and rmsg:
        ...
        url = re.search("(?P<url>https?://[^\s]+)", myString).group("url")
    if not url:
        return await edit_or_reply(event, "❆︙برو حط الرابط وانا انزله الك")
    catevent = await edit_or_reply(event, "❆︙صبر شوي ")
    reply_to_id = await reply_id(event)
...
async def kakashi(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await catevent.edit(" ❆︙روح فك البلوك الزكه عل بوت  @instasavegrambot")
            return
        await catevent.delete()
